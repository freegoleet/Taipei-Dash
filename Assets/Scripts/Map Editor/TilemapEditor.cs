...
namespace Traffic
{
    ...
    public class TilemapEditor : MonoBehaviour
    {
        ...
        public void NewTileHovered(Tile tile, Vector2Int newPos) {
            ...
            UpdateUINeighbors(tile);
        }

        public void UpdateUINeighbors(Tile tile) {
        ...
        private void HandleGameplayTile(Tile tile) {
            ...
            GridManager.TileManager.FitTileWithNeighbors((TileGameplay)tile);
            ...
            if (HoveredTile is TileRoad tileRoad == true) {
                switch (CurrentTileModType) {
                    case TileModType.Connections:
                        ...
                        UpdateUINeighbors(tileRoad);
                        break;
                    case TileModType.Crosswalk:
                        GridManager.TileManager.RoadUtils.ToggleCrosswalk(tileRoad);
                        break;
                    case TileModType.Stopline:
                        GridManager.TileManager.RoadUtils.ToggleStopline(tileRoad, ToggleType.Toggle, true);
                        break;
                    case TileModType.Pointer:
        ...
        public TrafficLight GetTrafficLight(Tile tile) {
            ...
            if(tile.gameObject.GetComponentInChildren<TrafficLight>() is TrafficLight tl) {
                return tl;
            }
            return null;
        }
        ...
        public void ConnectTrafficLights(TrafficLight trafficLight) {
            if (m_FirstTL == null) {
                ...
                m_FirstTL.Tile.ToggleHighlight(true);
                return;
            }

            if(trafficLight.Tile.Facing == m_FirstTL.Tile.Facing ||
                TrafficUtilities.ReverseDirections(trafficLight.Tile.Facing) == m_FirstTL.Tile.Facing) {
                trafficLight.SyncNew(m_FirstTL);
            }
            else
            {
                trafficLight.UnsyncNew(m_FirstTL);
            }

            m_FirstTL.SyncTimers();
            m_FirstTL.Tile.ToggleHighlight(false);
            m_FirstTL = null;
        }
        ...
        public void ShowTrafficLightConnections() {
            ...
            foreach (TrafficLight light in trafficLight.UnsyncedLights) {
